...
float SDF(vec3 pos) {
  ...
  float sphere1 = hollow(SDFSphere(vec3(25.0, 0.0, 0.0), 4.0, pos), 0.01); 
  float sphere2 = hollow(SDFSphere(vec3(45.0, 0.0, 0.0), 5.0, pos), 0.25); 
  float sphere3 = hollow(SDFSphere(vec3(55.0, 0.0, 0.0), 5.0, pos), 0.25); 

  dist = min(sphere1, dist);
  dist = min(smoothMin(sphere2, sphere3, 4.0), dist);
...
float rayMarchOcclusion(vec3 startPos, vec3 n, float k, float stepSize, int steps) {
  float ao = 1.0;
  for (int i = 1; i <= steps; ++i) {
    vec3 curPos = startPos + i * stepSize * n;
    float signedDist = SDF(curPos);
    ao -= k / pow(2.0, i) * (i * stepSize - signedDist);
  }
...
vec3 subsurfaceColor(
    ...
    float scale,
    float distortion) {
  vec3 scatterDir = rayDir + normal * distortion;
  float lightReachingEye = 
      pow(clamp(dot(rayDir, scatterDir), 0.0, 1.0), glow) * scale;
  float attenuation = max(0.0, dot(normal, rayDir) + 1.0);

  float ambient = 0.03;
  float totalLight = attenuation * (lightReachingEye + ambient) * thinness;
  return baseColor * envSample * totalLight;
}

Material SDFMaterial(vec3 pos, vec3 rayDir, vec3 normal, vec3 envSample) {
  Material mat;
  mat.metallic = 0.0;
  mat.roughness = 0.1;
  mat.occlusion = rayMarchOcclusion(pos, normal, 5.0, 0.1, 10);
  ...
  float thinness = rayMarchOcclusion(pos, rayDir, 2.0, 0.085, 5);
  mat.emission = 
      (1.0 - mat.metallic) * 
      subsurfaceColor(rayDir, 
        normal, 
        mat.color, 
        envSample,
        thinness,
